...
before DataFusion planning. It also adds a default no-op physical-plan observer, called immediately
before execution, so the course shell can report when its vector index is selected without teaching
the vendored CLI about course-owned plan nodes.
...
3. Reapply the bounded `validate_sql` and physical-plan observer hooks plus the documented removals;
   do not copy the upstream binary, cloud/object-store setup, tests, or unrelated dependencies by
   default.
